...
export default function App() {
  ...
  const [useCount, setUseCount] = useState(() => parseInt(localStorage.getItem('refract_uses') || '0'));
    ...
    <div style={{ background: "#0c0c0b", minHeight: "100vh", color: "#f0ece4", fontFamily: "'DM Sans', sans-serif" }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .plat{padding:9px 18px;border-radius:4px;font-size:13px;font-weight:500;cursor:pointer;border:1px solid #252523;background:#141412;color:#666;transition:all .15s}
        .plat:hover{border-color:#3a3a38;color:#aaa}
        .plat.on{background:#f0ece4;border-color:#f0ece4;color:#0c0c0b;font-weight:600}
        .tone{padding:6px 13px;border-radius:3px;font-size:12px;cursor:pointer;border:1px solid #252523;background:transparent;color:#555;transition:all .15s}
        ...
        .gen:hover{background:#f0ca6b;transform:translateY(-1px)}
        .gen:disabled{background:#252523;color:#3a3a38;cursor:not-allowed;transform:none}
        ...
        .cpbtn{padding:7px 16px;border-radius:3px;border:1px solid #252523;background:transparent;color:#666;font-size:12px;cursor:pointer;transition:all .15s}
        .cpbtn:hover{border-color:#3a3a38;color:#aaa}
        .cpbtn.done{border-color:#22c55e44;color:#22c55e;background:#22c55e0d}
        .exbtn{padding:5px 12px;border-radius:3px;border:1px solid #252523;background:transparent;color:#555;font-size:12px;cursor:pointer}
        .exbtn:hover{border-color:#3a3a38;color:#aaa}
        .divider{width:100%;height:1px;background:linear-gradient(90deg,transparent,#252523,transparent)}
        ...
        .upgrade-btn{width:100%;padding:14px;background:#e8b84b;color:#0c0c0b;border:none;border-radius:4px;font-size:14px;font-weight:700;cursor:pointer;letter-spacing:.3px;text-transform:uppercase;margin-bottom:12px}
        .upgrade-btn:hover{background:#f0ca6b}
      `}</style>
      ...
      {showPaywall && (
        <div className="paywall-overlay" onClick={() => setShowPaywall(false)}>
          <div className="paywall-box" onClick={e => e.stopPropagation()}>
            <div style={{fontFamily:"'Instrument Serif',serif",fontSize:26,marginBottom:12,letterSpacing:"-0.5px"}}>
              You've used your 3 free generations ✦
            </div>
            <p style={{fontSize:14,color:"#666",lineHeight:1.7,marginBottom:24}}>
              Upgrade to Refract Pro for unlimited content repurposing across all platforms.
            </p>
            <div style={{background:"#0c0c0b",border:"1px solid #252523",borderRadius:6,padding:"20px",marginBottom:24}}>
              <div style={{fontFamily:"'Instrument Serif',serif",fontSize:36,color:"#e8b84b",fontStyle:"italic"}}>
                $29<span style={{fontSize:14,color:"#555",fontStyle:"normal"}}>/month</span>
              </div>
              <div style={{fontSize:13,color:"#555",marginTop:8}}>Unlimited generations · All 5 platforms · All tones</div>
            </div>
            <button className="upgrade-btn" onClick={() => window.open(STRIPE_LINK, '_blank')}>
      ...
      <div style={{maxWidth:860,margin:"0 auto",padding:"0 40px 100px",position:"relative",zIndex:1}}>
        <div style={{border:"1px solid #252523",borderRadius:6,overflow:"hidden",background:"#0f0f0d"}}>
          ...
          <textarea
            ...
            placeholder="Paste your blog post, idea, video script, podcast notes — anything. Refract handles the rest..."
            rows={8}
        ...
        {error && (
          <div style={{marginTop:12,padding:"11px 16px",background:"#1a0808",border:"1px solid #4a1515",borderRadius:4,color:"#e05252",fontSize:13}}>{error}</div>
        )}
          ...
          <div ref={resultsRef} className="fade-up" style={{marginTop:20}}>
            <div style={{border:"1px solid #252523",borderRadius:6,overflow:"hidden",background:"#0f0f0d"}}>
              <div style={{padding:"10px 14px",borderBottom:"1px solid #181816",display:"flex",gap:4,flexWrap:"wrap",alignItems:"center"}}>
                ...
                <div style={{marginLeft:"auto",fontSize:10,color:"#22c55e",letterSpacing:".4px",textTransform:"uppercase",fontWeight:600}}>
                  ✓ {Object.keys(results).length} posts ready
                </div>
              </div>
                ...
                <div style={{padding:"24px 22px"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
                    <span style={{fontSize:10,color:"#3a3a38",letterSpacing:".6px",textTransform:"uppercase",fontWeight:600}}>
                      {PLATFORMS.find(p=>p.id===activeTab)?.label} · {TONES.find(t=>t.id===tone)?.label} tone
                    </span>
        ...
        <div style={{maxWidth:860,margin:"0 auto"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))",gap:12}}>
            {[
              ...
              {name:"Pro",price:79,desc:"Daily publishers",perks:["Unlimited repurposes","All 5 platforms","All 5 tones","Priority speed","Bulk mode"],hot:true},
              ...
              <div key={plan.name} style={{border:`1px solid ${plan.hot?"#e8b84b55":"#252523"}`,borderRadius:6,padding:"26px 22px",background:plan.hot?"#0f0d08":"#0f0f0d",position:"relative",overflow:"hidden"}}>
                {plan.hot&&<div style={{position:"absolute",top:14,right:14,fontSize:10,fontWeight:700,color:"#e8b84b",textTransform:"uppercase"}}>Popular</div>}
                ...
                <button onClick={()=>window.open(STRIPE_LINK,'_blank')} style={{width:"100%",padding:"11px",borderRadius:4,border:`1px solid ${plan.hot?"#e8b84b":"#252523"}`,background:plan.hot?"#e8b84b":"transparent",color:plan.hot?"#0c0c0b":"#555",fontSize:12,fontWeight:700,cursor:"pointer",letterSpacing:".4px",textTransform:"uppercase"}}>
                  Start free trial
                </button>
              </div>
            ))}
          </div>
          <p style={{fontSize:12,color:"#3a3a38",marginTop:18,textAlign:"center"}}>7-day free trial · No credit card required · Cancel anytime</p>
        </div>
      </div>
      ...
      <div style={{borderTop:"1px solid #181816",padding:"22px 40px",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:12}}>
        ...
        <span style={{fontSize:11,color:"#3a3a38",letterSpacing:".3px",textTransform:"uppercase"}}>Powered by AI · Built for global creators</span>
      </div>
    </div>
  );
}
